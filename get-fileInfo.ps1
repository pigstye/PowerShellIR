...
process {
	...
	#$files = Get-ChildItem $DriveSpec -Recurse -Force | where-object {$_.lastwritetime -gt (get-date).adddays(-$daystoCheck)-or $_.creationtime -gt (get-date).adddays(-$daystoCheck)} -ErrorAction SilentlyContinue
	$files = Get-ChildItem $DriveSpec -Recurse -Force -ErrorAction SilentlyContinue
